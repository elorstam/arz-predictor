...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 3 {
        return Err("usage: goal_v3_export <production-database> <output-jsonl>".into());
    }
    ...
    let rows = c.prepare("SELECT id,kickoff_at,scheduled_local_date,home_team_id,away_team_id,competition_id,final_home_goals,final_away_goals FROM main.matches WHERE scheduled_local_date BETWEEN '2026-09-12' AND '2026-09-14' ORDER BY julianday(kickoff_at),id")?.query_map([],|r| Ok((r.get::<_,i64>(0)?,r.get::<_,String>(1)?,r.get::<_,String>(2)?,r.get::<_,i64>(3)?,r.get::<_,i64>(4)?,r.get::<_,i64>(5)?,r.get::<_,Option<i64>>(6)?,r.get::<_,Option<i64>>(7)?)))?.collect::<Result<Vec<_>,_>>()?;
